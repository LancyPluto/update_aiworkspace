...
关联文档：[统一 API 管理 — 开发文档](./统一API管理-开发文档.md)（厂商账户与模型层）、[后台工具 JSON 配置说明](./后台工具JSON配置说明.md)（工具字段 schema）、[同系列模型版本表单配置规范](./同系列模型版本表单配置规范.md)（表单内 `model` 选型）、[可灵参考列表表单配置说明](./可灵参考列表表单配置说明.md)（可灵 imageList / videoList / elementList）、[模型配置.md](../模型配置.md) / [可灵模型配置.md](../可灵模型配置.md)（模型运营手册）。
