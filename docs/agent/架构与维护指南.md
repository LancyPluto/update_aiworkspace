...
- **原生 function-calling 自决策**：`agent` 节点用 `model_client.chat_turn(messages, tools, tool_choice="auto")` 让模型自行决定聊天还是调工具。
- **Legacy / DeepAgents 路径**：使用 `app/routing/` 管道（StateGuard → LLMClassifier → ToolResolver → PolicyValidator），**不再**用中文关键词做意图短路；附件场景由 LLM 结合 `attachmentSignals` 判断「参考生成 vs 文件分析」。
...
多模态 Agent 的续作、参考图和 `@图片` 交互遵循「状态注入 + 语义消解 + 指针解析」架构，禁止回退到业务代码关键词拦截。

**禁止事项：**

- 不允许在 Router、Handler、Resolver 或 Bridge 层用 `if "刚刚" in text`、正则、中文白名单等方式识别「刚刚 / 上一张 / 原图 / 这张」并强行填参。
- 不允许把 `@图片1`、inline chip 文案、素材名称当作真实图片传给 Worker。
- 不允许把 `base_image_url` 和 `reference_images` 重新混成一个 Agent 首选的 `image` 字段。
- 不允许把「电影级、构图、上一张图、刚刚生成的图」等风格/上下文词当作视频意图；只有明确要求视频输出时才走视频工具。

**正确链路：**

1. [session_state.py](../../agent-service/app/runtime/session_state.py) 从结构化 `recentToolCalls.mediaUrls / argumentsJson / resultJson` 水合最新生成图和原始 prompt，作为 `<SessionState>` 注入模型消息。
2. LLM 根据 `<SessionState>` 和工具 schema 自行完成语义级指代消解。续作编辑应填 `base_image_url=<latest_generated_image.url>`，显式 `@` 参考图进入 `reference_images`。
3. [attachment_catalog.py](../../agent-service/app/core/attachment_catalog.py) 维护 `@图片` / inline chip / `contentParts` 的统一指针目录，并在工具创建任务前通过 `resolve_media_argument_pointers` 解析成具体 URL。
4. [AgentToolDescriptorServiceImpl.java](../../backend/src/main/java/com/aiminilab/aitoolmarket/agent/service/impl/AgentToolDescriptorServiceImpl.java) 为 GPT/OpenAI 图片工具暴露语义化 schema：`prompt`、`base_image_url`、`reference_images`。
5. [image_generation_handler.py](../../worker/handlers/image_generation_handler.py) 按 `base_image_url` → `reference_images` → legacy 字段的顺序组装图片；第 1 张永远是底图，后续才是参考图。

**Prompt 继承规则：**

- 当用户要求修改、微调、换脸、修脸、改姿势或调整已有图片时，系统提示要求模型复用 `<SessionState.latest_generated_image.prompt>`，只追加用户的最小增量要求。
- 未经用户授权，不得重写画风、服装、光线、背景、镜头、构图或题材。
- 如果用户只上传/引用一张图并要求全新生成，不应自动使用上一轮图片作为 `base_image_url`；这个判断交给 LLM 结合 `<SessionState>` 和用户语义完成。

详见根目录 [agent.md](../../agent.md) 和参考图排障文档 [参考图解析链路与多图保留-2026-06-13.md](../参考图解析链路与多图保留-2026-06-13.md)。
...
| `app/runtime/session_state.py` | 会话状态水合：最新生成图、原始 prompt、`<SessionState>` 注入规则 |
| `app/runtime/agent_graph/` | 多步 graph 引擎（engine/state/tool_specs/prompts） |
| `app/routing/` | 意图路由管道（StateGuard / LLMClassifier / ToolResolver / PolicyValidator） |
| `app/core/attachment_catalog.py` | `@图片` / inline chip 指针目录、`contentParts`、工具参数 URL 解析 |
| `app/core/intent_router.py` | 兼容 shim（`IntentRouter` = `StateGuard`） |
...
| `app/tools/registry.py` | 工具匹配和保守输出模态判断 |
...
| `agent/service/impl/AgentToolDescriptorServiceImpl.java` | Agent 工具 schema；GPT/OpenAI 图片工具必须暴露 `base_image_url` / `reference_images` |
...
- 多模态上下文回归：`tests/test_session_state.py`、`tests/test_attachment_catalog.py`、`tests/test_user_attachment_priority.py`、`tests/test_backend_tool_bridge.py`、`tests/test_tool_registry.py`，以及 Worker 的 `worker/tests/test_image_generation_handler_references.py`。
...
| 多模态状态水合 / Prompt 继承 | `session_state.py`、三套 runtime 消息构造、本文第 8 节、`tests/test_session_state.py` |
| `@图片` / inline chip / contentParts 指针 | `attachment_catalog.py`、`backend_tool.py`、`AgentRunServiceImpl.java`、前端 mention editor、参考图链路文档、`tests/test_attachment_catalog.py` |
| 图片工具 schema | `AgentToolDescriptorServiceImpl.java`、`backend_tool.py`、Worker 图片 handler、本文第 8 节、`tests/test_backend_tool_bridge.py` |
| 输出模态判断 | `registry.py`、路由测试；禁止用上下文代词或风格词触发视频 |
