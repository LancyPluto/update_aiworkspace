...
| Agent 编排层 | `agent-service` | 意图路由、上下文注入、受控工具调用、文件与记忆运行时 |
...
sequenceDiagram
  ...
  participant Q as RabbitMQ/Redis
...
输入来自用户端、管理后台或内部服务。Controller 完成权限与参数入口，Service 处理业务规则和状态机，Mapper/MyBatis-Plus 访问 MySQL。异步任务由 `TaskQueuePublisher` 投递到 RabbitMQ/Redis，Worker 通过内部接口回写任务状态。Agent Service 只通过 `/api/internal/v1/agent` 相关接口读取上下文或提交运行结果。
...
| 目录/文件 | 说明 |
| --- | --- |
| `main.py` | 根据 `TASK_QUEUE_BACKEND` 选择 RabbitMQ 或 Redis 消费器 |
| `config.py` | 统一读取 Redis、RabbitMQ、后端内部 API、供应商 Key、媒体路径等环境变量 |
...
Worker 从 RabbitMQ/Redis 拉取任务消息，通过后端内部 API 获取任务上下文和模型配置，调用对应供应商生成结果。文本结果直接回写，媒体类结果先写入 `data/generated-media`，再以公开路径回写后端，由用户端展示。

### 3. Agent Service

#### 功能定位

`agent-service` 是独立 FastAPI 服务，负责 Agent 意图识别、工具选择、上下文组织、工作区文件/记忆注入以及受控工具调用。它不拥有业务数据，运行结果和审计记录仍写回后端。
...
后台页面通过后端 admin API 读写工具、字段 schema、模型供应商账号、系统设置和任务信息。复杂配置变更最终落入 MySQL，并影响用户端展示、Worker 执行上下文或 Agent 可用工具集合。
...
| 优先级 | 建议 | 说明 |
| --- | --- | --- |
| P0 | 梳理敏感文件管理 | 仓库中存在证书目录和 `.env`，应确认真实密钥未纳入版本控制，并建立 `.env.example`、密钥轮换和部署密钥规范 |
...
2. 阅读本文，理解服务边界和主数据流。
3. 阅读 `docs/api/openapi.yml` 和 `docs/统一API管理-开发文档.md`，理解接口与模型供应商配置。
