...
> 工作流工具的上线、版本发布、`/agents` 与 `/agent` 双入口语义，以 [`/agents` 工作流工具 P0 V2 方案](./superpowers/specs/2026-07-13-workflow-tools-p0-v2-design.md) 为准。本文重点保留 AI 漫剧制作流程和业务交付范围。
...
- 工作流版本管理和回滚。
- 工具列表提供唯一的上线/下线总开关：上线自动校验并发布当前已保存草稿，下线停止接受新任务但不终止运行中任务。
- 节点级人工审核开关。
...
- 工作流画布入口。
- 唯一的上线/下线开关；不要求运营另外开启工作流执行位或 Agent 展示位。
- 模型绑定状态。
- 近 7 天生成量、失败率、平均成本。

上线后，同一个 AI 漫剧工作流工具同时供 `/agents` 页面直接启动和 `/agent` 聊天智能体调用。下线后两个入口都拒绝新任务，已经开始的任务继续使用其锁定版本完成。
...
- “保存”只更新草稿；在线工具点击“发布更新”后，后续新任务使用新版本，运行中任务继续使用原版本。
...
- 离线工具点击上线后，系统自动校验并发布当前已保存草稿，且 `/agents` 与 `/agent` 同时可用。
- 工具下线后两个入口都不再创建新任务，运行中的任务仍可完成。
- 用户能提交主题并生成结构化策划案、分镜、角色卡、图/视频 Prompt。
- 任务结果能展示 Markdown 和资源链接。
- 工作流保存草稿、发布更新、版本记录和恢复可用；保存草稿不会影响当前正式版本。
