...
FILES = [
    ...
    "backend/src/main/java/com/aiminilab/aitoolmarket/credit/alipay/AlipayPagePayResponse.java",
...
def patch_remote_env(sftp: paramiko.SFTPClient) -> None:
    ...
    content = re.sub(r"^ALIPAY_PAY_MODE=.*\n?", "", content, flags=re.MULTILINE)
    with sftp.open(env_path, "w") as remote_file:
        remote_file.write(content.replace("\r\n", "\n").encode("utf-8"))
    print("removed remote ALIPAY_PAY_MODE (backend fixed to PAGE)")
...
def main() -> int:
    ...
    print((stdout.read().decode("utf-8", errors="replace") + stderr.read().decode("utf-8", errors="replace"))[-8000:])
    ssh.close()
    return 0
