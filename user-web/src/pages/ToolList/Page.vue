        ...
        <div class="rounded-3xl border border-white/8 bg-white/[0.04] p-5">
          ...
          <div class="mt-4 flex gap-3 overflow-hidden">
            <RouterLink
              ...
              :key="tool.id"
              :to="{ path: '/dashboard', query: { modality: normalizeModality(tool.outputModality), tool: tool.id } }"
      ...
      <div v-else class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
        ...
        >
          ...
          <div v-else class="flex min-h-[320px] flex-col px-5 pb-5 pt-6">
            <div
              class="mb-5 flex h-[92px] w-[92px] items-center justify-center overflow-hidden rounded-3xl ring-1 ring-white/10 transition group-hover:ring-primary/50"
