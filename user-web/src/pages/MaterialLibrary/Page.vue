...
<template>
  <AppShell title="素材库" description="从已完成任务中沉淀可复用的生成结果">
    <div class="px-6 py-6">
      <div v-if="error" class="mb-4 rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
        {{ error }}
      </div>
      ...
      <div v-else-if="materials.length === 0" class="rounded-lg border border-dashed border-border bg-card px-6 py-12 text-center">
        ...
        <RouterLink :to="userRoutes.toolList" class="mt-4 inline-flex rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">
          去创建任务
        </RouterLink>
      </div>

      <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <article v-for="item in materials" :key="item.taskId" class="rounded-lg border border-border bg-card p-5">
          ...
          <RouterLink :to="userRoutes.taskResult(String(item.taskId))" class="mt-4 inline-flex items-center gap-1 text-xs font-medium text-primary">
