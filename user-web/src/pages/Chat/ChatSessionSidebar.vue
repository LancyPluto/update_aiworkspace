...
async function handleDelete(session: ChatSession, event: MouseEvent) {
  event.stopPropagation()
  const confirmed = await confirmDelete({
    title: "删除会话",
    itemName: session.title || "新对话",
  })
  if (!confirmed) return
  emit("delete", session, event)
}
</script>
...
<template>
  ...
  >
    <div class="flex h-full w-64 flex-col p-3">
      <button
        type="button"
        class="inline-flex h-9 w-full items-center justify-center gap-2 rounded-lg bg-primary text-sm font-medium text-primary-foreground hover:opacity-90 transition-all"
        @click="emit('create')"
      >
        <Plus class="h-4 w-4" />
        新建会话
      ...
      <div v-else class="mt-3 min-h-0 flex-1 overflow-y-auto">
        ...
        <ul v-else class="space-y-1">
          <li
            ...
            :key="session.id"
            class="group flex items-stretch rounded-lg transition-colors"
            :class="session.id === activeSessionId ? 'bg-primary/15 shadow-sm' : 'hover:bg-secondary/80'"
          >
            <button
              type="button"
              class="flex min-w-0 flex-1 items-center gap-2 px-2.5 py-2 text-left text-sm transition-transform active:scale-98"
            ...
            <button
              type="button"
              class="flex w-8 shrink-0 items-center justify-center text-muted-foreground opacity-0 transition-all duration-200 hover:scale-110 hover:text-destructive group-hover:opacity-100"
